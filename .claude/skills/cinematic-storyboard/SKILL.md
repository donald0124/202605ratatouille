...
description: Cinematic Storyboard Generator — AI 電影分鏡生成工作流技能。當使用者說「幫我做分鏡」、「做電影分鏡」、「生成 storyboard」、「我要做 AI 影片分鏡」、「幫我把腳本轉成分鏡」、「cinematic storyboard」、「Seedance 分鏡」或任何需要將腳本／場景描述轉換為電影級分鏡板的情境時，請一定要使用此技能。此技能依照六步驟工作流引導使用者完成：腳本輸入 → 場景分析 → 分鏡生成 → 用戶確認循環 → 單格放大 → Seedance 2.0 提示詞輸出。
---
...
When this skill is invoked, your FIRST response must ALWAYS be:
...
Use the `draw` skill to generate ONE cinematic storyboard image with the following requirements:

### Storyboard Requirements

- Aspect ratio: 16:9 overall (`--size 1536x1024`)
- Total frames: 9
- Layout:
...
- Every frame must contain visible numbering: 1 through 9
...
When upscaling, use the `draw` skill with `--quality medium` and `--size 1536x1024` to regenerate the selected frame:
...
Generate a highly detailed cinematic video-generation prompt optimized for **Seedance 2.0**
